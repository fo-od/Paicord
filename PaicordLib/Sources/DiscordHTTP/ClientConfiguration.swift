...
public struct ClientConfiguration: Sendable {
  ...
  public struct CachingBehavior: Sendable {
    ...
    func getTTL(for identity: CacheableEndpointIdentity) -> Duration? {
      switch identity {
      case .api(let cacheableAPIEndpointIdentity):
      ...
      public static var `default`: Backoff {
        .basedOnHeaders(
          ...
          retryIfGreater: false,
            ...
            base: 0.2, coefficient: 0.5, rate: 2, upToTimes: 10)
        )
      }

      /// Returns the time needed to wait before the next retry, if any.
      @inlinable
      func waitTimeBeforeRetry(retriesSoFar: Int, headers: HTTPHeaders)
        -> Double?
      {
        switch self {
        case .constant(let constant):
          return constant
        case .linear(let base, let coefficient, let upToTimes):
          let times = retriesSoFar + 1
          let multiplyFactor = min(Int(upToTimes), times)
          let time = coefficient * Double(multiplyFactor) + base
          return time
        case .exponential(let base, let coefficient, let rate, let upToTimes):
          let times = retriesSoFar + 1
          let exponent = min(Int(upToTimes), times)
          let time = base + coefficient * pow(rate, Double(exponent))
          return time
        case .basedOnHeaders(let maxAllowed, let retryIfGreater, let elseBackoff):
  ...
  @inlinable
  func waitTimeBeforeRetry(retriesSoFar times: Int, headers: HTTPHeaders)
    -> Double?
  {
    switch self.retryPolicy?.backoff {
    case .some(let backoff):
...
extension HTTPHeaders {
  ...
  func resetOrRetryAfterHeaderValue() -> String? {
    self.first(name: "x-ratelimit-reset-after")
      ?? self.first(name: "retry-after")
  }
}
