...
class ClientConfigurationTests: XCTestCase {
  ...
  func testRetryPolicyLinearBackoff() throws {
    ...
    XCTAssertEqual(
      backoff.waitTimeBeforeRetry(retriesSoFar: 3, headers: [:]),
      base + 3 * coefficient
    )
    XCTAssertEqual(
      backoff.waitTimeBeforeRetry(retriesSoFar: 4, headers: [:]),
      base + 3 * coefficient
    )
    XCTAssertEqual(
      backoff.waitTimeBeforeRetry(retriesSoFar: 5, headers: [:]),
      base + 3 * coefficient
    )
    XCTAssertEqual(
      backoff.waitTimeBeforeRetry(retriesSoFar: 10, headers: [:]),
  ...
  func testRetryPolicyExponentialBackoff() throws {
    ...
    XCTAssertEqual(
      backoff.waitTimeBeforeRetry(retriesSoFar: 0, headers: [:]),
      base + coefficient * rate
    )
    XCTAssertEqual(
      backoff.waitTimeBeforeRetry(retriesSoFar: 1, headers: [:]),
      base + coefficient * (pow(rate, 2))
    )
    XCTAssertEqual(
      backoff.waitTimeBeforeRetry(retriesSoFar: 2, headers: [:]),
      base + coefficient * (pow(rate, 3))
    )
    XCTAssertEqual(
      backoff.waitTimeBeforeRetry(retriesSoFar: 3, headers: [:]),
  ...
  func testRetryPolicyHeadersBackoffWithElse() throws {
    ...
    do {
      ...
      XCTAssertEqual(
        backoffWait(retriesSoFar: 10, headers: headers),
        maxAllowed
      )
      XCTAssertEqual(
        backoffWait(retriesSoFar: Int.max - 1, headers: headers),
        maxAllowed
      )
    }
  ...
  func testCacheClient() async throws {
    ...
    /// Basic caching but test a user API route
    do {
      let cache = ClientCache()
      let response = DiscordHTTPResponse(
        host: "something.else",
      ...
      let item = ClientCache.CacheableItem(
        ...
        queries: [("with_mutual_guilds", "true")],
      )
      await cache.save(response: response, item: item, ttl: .seconds(5))
      let fromCache = await cache.get(item: item)
      XCTAssertEqual(response, fromCache)
    ...
    for length in (1...19) {
      for _ in 0..<100 {
        ...
        if let number = UInt(String(numberString)) {
          ...
          XCTAssertEqual(
            header.extractAppIdIfAvailable()?.rawValue,
            "\(number)"
          )
        }
      }
...
extension DiscordHTTPResponse: Equatable {
  public static func == (lhs: DiscordHTTPResponse, rhs: DiscordHTTPResponse)
    -> Bool
  {
    lhs.host == rhs.host && lhs.status == rhs.status
