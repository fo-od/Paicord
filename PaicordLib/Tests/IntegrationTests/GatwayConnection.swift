...
class GatewayConnectionTests: XCTestCase, @unchecked Sendable {
  ...
  @available(
    *, deprecated, message: "To avoid deprecation warnings for 'makeEventsParseFailureStream'"
  )
  func testConnect() async throws {
    ...
    Task {
      for await event in await bot.events {
        if case .ready(let ready) = event.data {
  ...
  func testGatewayRequests() async throws {
    ...
    let presence = try XCTUnwrap(
      guild.presences.first(where: { $0.user?.id == Constants.botId }), "\(guild)")
