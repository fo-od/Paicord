...
class DiscordClientTests: XCTestCase {
  ...
  func testGuildWithCreatedGuild() async throws {
    ...
    case .jsonError(let jsonError) where jsonError.code == .invalidGuild:
      break
  ...
  func testGuildMembers() async throws {
    ...
    case .jsonError(let jsonError) where jsonError.code == .invalidOAuth2AccessToken:
      break
    case .none, .badStatusCode, .jsonError:
  ...
  func testGuildIntegrations() async throws {
    ...
    case .jsonError(let jsonError) where jsonError.code == .unknownIntegration:
      break
    case .none, .badStatusCode, .jsonError:
  ...
  func testGuildOthers() async throws {
    ...
    case .jsonError(let jsonError) where jsonError.code == .missingAccess:
      break
    case .none, .badStatusCode, .jsonError:
  ...
  func testUser() async throws {
    ...
    do {
      ...
      if case .badStatusCode(let response) = error,
  ...
  func testDMs() async throws {
    ...
    case .jsonError(let jsonError) where jsonError.code == .invalidFormBodyOrInvalidContentType:
      break
    default:
    ...
    case .jsonError(let jsonError) where jsonError.code == .missingPermissions:
      break
    default:
      XCTFail("Unexpected error: \(String(describing: createGroupDmError))")
    }
