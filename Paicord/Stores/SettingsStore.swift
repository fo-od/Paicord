...
class SettingsStore: DiscordDataStore {
	...
	// MARK: - Settings State
	var userSettings: DiscordProtos_DiscordUsers_V1_PreloadedUserSettings = .init()
	var frecencySettings: DiscordProtos_DiscordUsers_V1_FrecencyUserSettings = .init()
	...
	private func handleUserSettingsUpdate(_ update: Gateway.UserSettingsProtoUpdate) {
	...
	private func mergeUserSettings(with newSettings: DiscordProtos_DiscordUsers_V1_PreloadedUserSettings) {
		
	}
	
	private func mergeFrecencySettings(with newSettings: DiscordProtos_DiscordUsers_V1_FrecencyUserSettings) {
	...
	func requestUserSettingsModify(updateType: UpdateType = .frequent, immediate: Bool = false) {
		...
		userSettingsTimer = Timer.scheduledTimer(withTimeInterval: delay, repeats: false) { [weak self] _ in
			Task { @MainActor in
	...
	func requestFrecencySettingsModify(updateType: UpdateType = .daily, immediate: Bool = false) {
		...
		let delay = delayInterval(for: updateType)
		frecencySettingsTimer = Timer.scheduledTimer(withTimeInterval: delay, repeats: false) { [weak self] _ in
			Task { @MainActor in
	...
	func updateUserSetting<T>(_ keyPath: WritableKeyPath<DiscordProtos_DiscordUsers_V1_PreloadedUserSettings, T>, to value: T, updateType: UpdateType = .frequent) {
	...
	func updateFrecencySetting<T>(_ keyPath: WritableKeyPath<DiscordProtos_DiscordUsers_V1_FrecencyUserSettings, T>, to value: T, updateType: UpdateType = .daily) {
